 ...
 * 
 * (hev-socks5-tunnel functionality has been safely removed and disabled).
 * [teardown] is maintained to clean up any legacy stale rules on the device.
 */
object RootProxyManager {
    ...
    fun startClientSharing(context: Context): Boolean {
        teardown(context)
        val script = buildTun2socksSetup(context, captureDeviceTraffic = false, forceLanShare = true) ?: return false
        val result = RootShell.runScript(context, "setup_rules.sh", script)
        if (!result.success) {
    ...
    private fun buildTun2socksSetup(
        ...
        // **ဘာကြောင့်ဖြုတ်တာလဲ - HevTun (hev-socks5-tunnel) binary ကို မသုံးတော့ပါဘူး။**
        // **ဒါကြောင့် ဒီ function က setup script ကို ထုတ်ပေးတော့မှာ မဟုတ်ဘဲ null ကို return ပြန်ပါတယ်။**
        // **ကျန်တဲ့ function တွေ၊ import တွေ အကုန်လုံးကို မူရင်းအတိုင်း ချန်ထားပါတယ်။**
        
        LogUtil.w(AppConfig.TAG, "RootProxyManager: HevTun (tun2socks) is removed. Setup safely aborted.")
        return null
    }
